...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const mapUserObj = (userObj: any): User => ({
      ...userObj,
    ...
    const fetchUser = async (userId: string) => {
      ...
      if (data) {
        setCurrentUser(mapUserObj(data));
      } else {
        console.error("DB dan foydalanuvchini olib bo'lmadi:", error);
        
        // PGRST116 means 0 rows returned for single()
        if (error?.code === 'PGRST116') {
          console.log("Foydalanuvchi auth dan o'tgan, lekin users jadvalida yo'q. Uni yaratamiz...");
          ...
          if (authData?.user) {
             ...
             const { data: insertedData, error: insertError } = await supabase.from('users').insert(newUser).select().single();
             if (insertedData) {
               setCurrentUser(mapUserObj(insertedData));
               setAuthLoading(false);
               return;
             } else {
                console.error("Yangi foydalanuvchini yaratib bo'lmadi:", insertError);
             }
          }
        }

        if (error?.code === '42501' && error?.message.includes('permission denied for schema public')) {
          showToast("XATOLIK: Supabase bazasida ruxsatlar yo'q. SQL orqali ruxsatlarni tiklang!");
        } else {
          showToast(`Xatolik: ${error?.message || "Foydalanuvchi ma'lumotlari topilmadi"}`);
        }
        setCurrentUser(null);
      }
      setAuthLoading(false);
  ...
  const fetchData = async () => {
    try {
      const [
        ...
        messagesRes, mistakesRes, certsRes, progressRes, notificationsRes
      ] = await Promise.all([
  ...
  const addSpotlight = async (spotlight: AchievementSpotlight) => {
    await supabase.from('spotlights').insert({
      employee_name: spotlight.employeeName,
      employee_avatar: spotlight.employeeAvatar,
      position: spotlight.position,
      store_name: spotlight.storeName,
  ...
  const updateSpotlight = async (id: string, spotlight: Partial<AchievementSpotlight>) => {
    ...
    // Clean up undefined
    Object.keys(updated).forEach(key => updated[key as keyof typeof updated] === undefined && delete updated[key as keyof typeof updated]);
    
    await supabase.from('spotlights').update(updated).eq('id', id);
    fetchData();
  };
  ...
  const sendChatMessage = async (text: string, fileAttachment?: any) => {
    ...
    await supabase.from('chat_messages').insert({ text: text, channel_id: activeChannelId, sender_id: currentUser.id });
